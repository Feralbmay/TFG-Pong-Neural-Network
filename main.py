...
HITS_GOAL = 1000  # Nombre de cops objectiu
...
def main():
    ...
    while(max_hit < HITS_GOAL):
        ...
        max_hit = hits[0]
        if (max_hit > max_hit_Global):
            print('ups')
            max_hit_Global = max_hit
            if (max_hit_Global > HITS_GOAL / 10):
                print('ups2')
                pickle.dump(best_nets[-1], open("../../Xarxes/xarxaAN_AUX.nn", "wb"), pickle.HIGHEST_PROTOCOL)
        ...
        if(hits[0] == 0):
            ...
            for i, n in enumerate(nets):
                if(i< NUM_XARXES * 0.8):
                    ...
                    nets[i] = Neural.Network(sizes)

        print("/ Hits", best_hits[-1])

    pickle.dump(best_nets[-1],open( "../../Xarxes/xarxaAN_2.nn", "wb" ), pickle.HIGHEST_PROTOCOL)
